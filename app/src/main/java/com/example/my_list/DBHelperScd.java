...
interface DBContractScd {
    static final String TABLE_NAME="PWD_SCD";
...
class DBHelperScd extends SQLiteOpenHelper {
    static final String DB_FILE = "pwd_sd.db";
    static final int DB_VERSION = 1;

    DBHelperScd(Context context) {
        super(context, DB_FILE, null, DB_VERSION); // 세번째 인자 : cursor factory
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(DBContractScd.SQL_CREATE_TABLE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL(DBContractScd.SQL_DROP_TABLE);
        onCreate(db);
    }
}
