...
interface DBContractTO {
    static final String TABLE_NAME="PWD_TAKEONE";
...
class DBHelperTO extends SQLiteOpenHelper {
    static final String DB_FILE = "pwd_One.db";
    static final int DB_VERSION = 1;

    DBHelperTO(Context context) {
        super(context, DB_FILE, null, DB_VERSION); // 세번째 인자 : cursor factory
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(DBContractTO.SQL_CREATE_TABLE);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL(DBContractTO.SQL_DROP_TABLE);
        onCreate(db);
    }
}
